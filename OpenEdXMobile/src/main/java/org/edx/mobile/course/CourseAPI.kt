...
class CourseAPI @Inject constructor(
    ...
    val enrolledCoursesFromCache: Call<EnrollmentResponse>
        get() = courseService.getEnrolledCoursesFromCache(
            userPrefs.profile.username,
            config.apiUrlVersionConfig.enrollmentsApiVersion,
            config.organizationCode
        )
    ...
    fun getCourseList(page: Int): Call<Page<CourseDetail>> {
        val username = if (userPrefs.loginPrefs.iaUserLoggedIn())
            userPrefs.profile.username else null
    ...
    fun getCourseDetail(courseId: String): Call<CourseDetail> {
        ...
        return courseService.getCourseDetail(courseId, userPrefs.profile.username)
    }
    ...
    fun getCourseStructureWithoutStale(
        blocksApiVersion: String,
        courseId: String
    ): Call<CourseStructureV1Model> {
        return courseService.getCourseStructure(
    ...
    fun getCourseStructureFromCache(blocksApiVersion: String, courseId: String): CourseComponent {
     ...
     */
    fun resetCourseDates(courseBody: Map<String, String>): Call<ResetCourseDates> {
    ...
    fun getCourseStatusInfo(courseId: String): Call<CourseComponentStatusResponse> {
        return courseService.getCourseStatusInfo(userPrefs.profile.username, courseId)
    }
    ...
    abstract class GetCourseByIdCallback(
        ...
        override fun onResponse(
            courseResponses: EnrollmentResponse
        ) {
            courseResponses.enrollments.forEach { coursesResponse ->
                if (coursesResponse.course.id == courseId) {
